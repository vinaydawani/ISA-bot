...
class fun(commands.Cog):
    ...
    async def get_cat(self, ctx, r):
        if r.status_code == 200:
            ...
            return await send_embedded(
                ctx,
                f"Error {r.status_code} in fetching the image. Try again later please.",
            )

        embed = discord.Embed(
            title="A wild cat has appeared :cat:",
    ...
    async def get_dog(self, ctx, r):
        ...
        embed = discord.Embed(
            title="OMG a doggo :dog:", color=random.choice(self.bot.color_list)
        )
        embed.set_image(url=j[0]["url"])
        embed.set_footer(
